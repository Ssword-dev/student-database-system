...
class DashboardLayout extends Layout
{
    ...
    {
        $app_name = $this->getAttribute('app_name', 'Student Database');
        $display_name = $this->getAttribute('display_name', 'Guest');
        $page_title = $this->getAttribute('page_title', 'Dashboard');
        $nav_links = $this->getAttribute('nav_links', []);
        ...
        return <<<HTML
            ...
            <body class="min-h-screen min-w-screen bg-background text-text flex">
                <!-- Sidebar -->
                <aside class="min-h-screen w-(--sidebar-width) p-6 bg-background/110 outline-1 outline-text transition-all duration-75" id="sidebar" style="--sidebar-width: 250px">
                ...
                <main class="flex-1 p-8 overflow-y-auto">
                    ...
                    <div class="flex items-center justify-between mb-8">
                        ...
                        <div class="flex items-center gap-4">
                            <span class="text-muted-foreground">$display_name</span>
                            <form action="../auth/logout" method="POST" style="display: inline;">
                                <button type="submit" class="px-4 py-2 text-sm rounded-md border border-outline hover:bg-background/80 transition">
                                    log out
                                </button>
                            </form>
                        </div>
